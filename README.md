...
2. 点击“一键初始化”：自动生成设定、人物、世界观、大纲和第一章
3. 点击“生成下一章”持续写作
4. 在章节列表中选择章节，编辑、保存、重写或续写正文
5. 导出整本小说 Markdown
...
章节 Markdown 文件只保存小说正文。模型输出会被后端强制拆成两部分：

- `chapter_text`：纯小说正文，只显示在章节编辑区并写入 `chapters/chapter_XXX.md`
- `memory_update`：结构化后台记忆，只合并到 `memory.json`

`memory.json` 是后台记忆，不是小说正文。它会记录新增地点、人物、伏笔、已发生事件、线索、人物关系变化和当前剧情进度。Web 页面默认不会把这些字段显示在正文区；调试信息只放在折叠的“后台记忆/调试信息”区域。
